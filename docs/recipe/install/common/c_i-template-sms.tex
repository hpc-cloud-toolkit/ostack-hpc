...
\end{lstlisting} 
 % end_ohpc_run
	...
	Distribute munge keys to compute nodes, and then update the resource manager with HPC compute nodes.
...
\end{lstlisting} 
 % end_ohpc_run
	...
	One last step is to make sure ssh is working and enabled on the compute nodes. Updateand verify the permissions of ssh.

% begin_ohpc_run
% ohpc_validation_newline

\begin{lstlisting}[language=bash,keywords={}]
[ctrlr](*\#*) #Change file permissions in /etc/ssh to fix ssh into compute nodes
